# Структуры мышления

<div style="
  min-height: 360px;
  padding: 56px 48px;
  border-radius: 18px;
  background-image:
    linear-gradient(90deg, rgba(2, 10, 18, 0.82), rgba(2, 10, 18, 0.38)),
    url('/assets/images/home-wallpaper.png');
  background-size: cover;
  background-position: center;
  color: #f5f7fa;
">

# Структуры мышления

Открытый архив эссе, заметок, переводов и исследовательских материалов о научных понятиях, физических теориях и способах описания реальности.

</div>
Открытый архив эссе, заметок, переводов и исследовательских материалов о научных понятиях, физических теориях и способах описания реальности.

## Основные разделы

### [Физика](physics/)

- [Квантовая механика](physics/quantum-mechanics/)
- [Реляционная программа](physics/relational-program/)
- [Интересные публикации](physics/interesting-publications/)

### [Когнитология](cognitology/)

- [Нейрофизиология](cognitology/neurophysiology/)
- [Семиотика](cognitology/semiotics/)
- [Сознание](cognitology/consciousness/)
- [Искусственный разум](cognitology/artificial-reason/)
- [Интересные публикации](cognitology/interesting-publications/)
